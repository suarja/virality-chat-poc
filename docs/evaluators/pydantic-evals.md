...
*   [`Case`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case): A single test scenario corresponding to "task" inputs. Can also optionally have a name, expected outputs, metadata, and evaluators.
*   [`Dataset`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset): A collection of test cases designed for the evaluation of a specific task or function.
...
1.  You can add built-in evaluators to a dataset using the [`add_evaluator`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.add_evaluator) method.
...
Putting the above two examples together and using the more declarative `evaluators` kwarg to [`Dataset`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset):
...
1.  Create a [test case](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case) as above
2.  Also create a custom evaluator function as above
3.  Create a [`Dataset`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) with test cases, also set the [`evaluators`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluators) when creating the dataset
4.  Our function to evaluate.
5.  Run the evaluation with [`evaluate_sync`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate_sync), which runs the function against all test cases in the dataset, and returns an [`EvaluationReport`](about:blank/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport) object.
6.  Print the report with [`print`](about:blank/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport.print), which shows the results of the evaluation, including input and output. We have omitted duration here just to keep the printed output from changing from run to run.
...
5.  Case-specific evaluator with a focused rubric using [`LLMJudge`](about:blank/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.LLMJudge).
...
The [`EvaluatorContext`](about:blank/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluatorContext) includes a property called `span_tree` which returns a [`SpanTree`](about:blank/api/pydantic_evals/otel/#pydantic_evals.otel.SpanTree). The `SpanTree` provides a way to query and analyze the spans generated during function execution. This provides a way to access the results of instrumentation during evaluation.
...
Pydantic Evals allows you to generate test datasets using LLMs with [`generate_dataset`](about:blank/api/pydantic_evals/generation/#pydantic_evals.generation.generate_dataset).
...
4.  Call [`generate_dataset`](about:blank/api/pydantic_evals/generation/#pydantic_evals.generation.generate_dataset) to create a [`Dataset`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) with 2 cases confirming to the schema.
...
1.  Generate the [`Dataset`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) exactly as above.
...
Logfire has some special integration with Pydantic Evals traces, including a table view of the evaluation results on the evaluation root span (which is generated in each call to [`Dataset.evaluate`](about:blank/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate)):
...
This can be especially helpful when attempting to write evaluators that make use of the `span_tree` property of the [`EvaluatorContext`](about:blank/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluatorContext), as described in the [OpenTelemetry Integration](#opentelemetry-integration) section above.
